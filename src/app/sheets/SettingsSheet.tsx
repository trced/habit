...
import { activeHabits } from '../../lib/habits.ts'
import { downloadFile, exportFilename, parseFile, shareFile } from '../../lib/io.ts'
import { LICENCE_URL, REPO } from '../../lib/links.ts'
import type { Completion, Habit, Settings } from '../../lib/types.ts'
...
const CYCLES = [
  ...
  {
    key: 'colors',
    labelKey: 'app.settings.colors',
    values: ['shown', 'hidden'],
    valueKey: (v) => `app.settings.colors.${v}` as MessageKey,
  } satisfies Cycle<'colors'>,
  {
    key: 'weekends',
    labelKey: 'app.settings.weekends',
    values: ['distinct', 'plain'],
    valueKey: (v) => `app.settings.weekends.${v}` as MessageKey,
  } satisfies Cycle<'weekends'>,
  {
    key: 'summary',
    labelKey: 'app.settings.summary',
    values: ['shown', 'hidden'],
    valueKey: (v) => `app.settings.summary.${v}` as MessageKey,
  } satisfies Cycle<'summary'>,
] as const
...
export function SettingsSheet({
  ...
  const count = store.habits.length
  const tracked = activeHabits(store.habits).length
  ...
  const onPickFile = async (file: File): Promise<void> => {
    ...
    if (result.file.data.habits.length === 0) {
    ...
    setPending({
      state: 'ready',
      filename: file.name,
      habits: result.file.data.habits,
      completions: result.file.data.completions,
    })
  }
  ...
  return (
    <Sheet full label={t('app.settings.title')} onClose={onClose}>
      <div className="settings__head">
        <span className="t-title">{t('app.settings.title')}</span>
      ...
      <div className="settings__body">
        <section>
          ...
          {/* Sur un téléphone, la barre haute n'existe pas : c'est ici que
              l'on vient renommer, réordonner et archiver. */}
          <ActionRow
            name={t('app.settings.habits')}
            value={tp('app.settings.habitsValue', tracked)}
            onClick={onManage}
          />
          ...
          {pending.state === 'ready' ? (
            <div className="confirm">
              <p className="confirm__title">
                {tp('app.settings.importFound', pending.habits.length, {
              ...
              <div className="confirm__actions">
                ...
                <Button
                  onClick={() => {
                    const added = store.mergeIncoming(
                      pending.habits,
                      pending.completions,
                    )
                    setPending({ state: 'idle' })
                    onFlash(
                      added.habits === 0 && added.completions === 0
                        ? t('app.flash.importedNone')
                        : added.habits === 0
                          ? tp('app.flash.importedDays', added.completions)
                          : tp('app.flash.imported', added.habits),
                    )
                    onClose()
                  }}
                >
                ...
                <Button
                  ...
                  strong
                  onClick={() => {
                    store.replaceAll(pending.habits, pending.completions)
                    setPending({ state: 'idle' })
                    onFlash(tp('app.flash.replaced', pending.habits.length))
                    onClose()
                  }}
                >
